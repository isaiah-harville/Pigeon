...
final class DoubleRatchetTests: XCTestCase {
    ...
    private func makePair(sharedSecret: Data = Data(repeating: 0x42, count: 32),
                          maxSkip: Int = 1000) throws
        -> (alice: DoubleRatchetSession, bob: DoubleRatchetSession) {
        let bobKeys = DHKeyPair()
        let alice = try DoubleRatchetSession.initiator(sharedSecret: sharedSecret,
                                                       remotePublicKey: bobKeys.publicKey,
                                                       maxSkip: maxSkip)
        let bob = DoubleRatchetSession.responder(sharedSecret: sharedSecret,
                                                 selfKeyPair: bobKeys,
                                                 maxSkip: maxSkip)
        return (alice, bob)
    }
    ...
    func testTamperedHeaderRejected() throws {
        ...
        let forged = RatchetHeader(dhPublic: m.header.dhPublic,
    ...
    func testHeaderEncodingRoundTrip() throws {
        let header = RatchetHeader(dhPublic: Data(repeating: 0xAB, count: 32),
