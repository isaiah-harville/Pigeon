...
public final class SecureSession {
  ...
  public func writeHandshakeMessage() throws -> Data {
    ...
    let payload =
      (!isInitiator) ? (responderRatchetKey?.publicKey.rawRepresentation ?? Data()) : Data()
  ...
  private func finishIfReady() throws {
    ...
    if isInitiator {
      ...
      ratchet = try DoubleRatchetSession.initiator(
        sharedSecret: rootSecret, remotePublicKey: remoteKey)
    } else {
