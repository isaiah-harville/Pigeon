...
public enum Primitives {
    ...
    private static func deriveAEAD(messageKey: Data) throws -> (key: SymmetricKey, nonce: AES.GCM.Nonce) {
    ...
    public static func encrypt(plaintext: Data, messageKey: Data, associatedData: Data) throws -> Data {
        ...
        let sealed = try AES.GCM.seal(plaintext, using: key, nonce: nonce, authenticating: associatedData)
    ...
    public static func decrypt(ciphertext: Data, messageKey: Data, associatedData: Data) throws -> Data {
        let (key, nonce) = try deriveAEAD(messageKey: messageKey)
        do {
