...
public struct Fragmenter {
    ...
    public mutating func fragment(_ message: Data, maxPayloadPerFragment: Int) throws -> [Fragment] {
        ...
        let chunkCount = max(1, Int((message.count + maxPayloadPerFragment - 1) / maxPayloadPerFragment))
        ...
        for i in 0..<chunkCount {
            let end = message.index(offset, offsetBy: maxPayloadPerFragment, limitedBy: message.endIndex) ?? message.endIndex
            fragments.append(Fragment(messageID: id,
                                      index: UInt16(i),
...
public final class Reassembler {
    ...
    public func ingest(_ fragment: Fragment) throws -> Data? {
        ...
        if let existing = pending[fragment.messageID], existing.count == fragment.count {
            ...
            entry = Pending(count: fragment.count, fragments: [:], byteCount: 0, sequence: sequenceCounter)
        }
...
private extension Data {
    func readBigEndianUInt16(at index: Int) -> UInt16 {
