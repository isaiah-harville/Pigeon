...
final class FragmentationTests: XCTestCase {
    ...
    func testInterleavedMessages() throws {
        ...
        var got: [Data] = []
        let interleaved = zip(f1, f2).flatMap { [$0, $1] } + f1.dropFirst(f2.count) + f2.dropFirst(f1.count)
    ...
    func testOversizeMessageRejected() throws {
        ...
        let id: UInt16 = 7
        _ = try reassembler.ingest(Fragment(messageID: id, index: 0, count: 2, payload: randomData(150)))
        XCTAssertThrowsError(try reassembler.ingest(Fragment(messageID: id, index: 1, count: 2, payload: randomData(150)))) {
    ...
    func testConcurrentPendingBounded() throws {
        ...
        for id in 0..<10 {
            _ = try reassembler.ingest(Fragment(messageID: UInt16(id), index: 0, count: 2, payload: Data([0x01])))
        }
    ...
    func testReusedMessageIDResetsState() throws {
        ...
        let r1 = try reassembler.ingest(Fragment(messageID: 1, index: 0, count: 2, payload: Data([0xBB])))
        XCTAssertNil(r1)
        let r2 = try reassembler.ingest(Fragment(messageID: 1, index: 1, count: 2, payload: Data([0xCC])))
