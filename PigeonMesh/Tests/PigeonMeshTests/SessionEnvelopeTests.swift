...
final class SessionEnvelopeTests: XCTestCase {
    ...
    func testRoundTripHandshake() throws {
        let env = SessionEnvelope(type: .handshake, sender: id(0xA1), recipient: id(0xB2),
    ...
    func testRoundTripMessage() throws {
        let env = SessionEnvelope(type: .message, sender: id(0x11), recipient: id(0x22),
    ...
    func testDecodeRejectsBadVersion() {
        var bytes = SessionEnvelope(type: .message, sender: id(1), recipient: id(2), payload: Data()).encoded()
