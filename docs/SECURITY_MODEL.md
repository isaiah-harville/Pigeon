...
> see [How Pigeon Works](HOW_IT_WORKS.md). Per-flow sequence diagrams (with what
> each party's keys do, and what a relay can and cannot see) live in
> [`docs/diagrams/`](diagrams/) and are regenerated by
> [`generate_diagrams.py`](diagrams/generate_diagrams.py).
